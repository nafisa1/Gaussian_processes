...
noise = np.vstack(0.05*np.random.rand(50, 1))
noise_variance = np.var(noise)

# Generate test values (data)
# The posterior is generated using the test 
X_train = np.vstack(5*np.random.rand(3, 1))
train_noise = np.vstack(0.05*np.random.rand(3, 1))
Y_train = np.sin(0.9*X_train) + train_noise

kern = kernels.RBF()
m = regression.Regression(input_range, true_f, X_train, Y_train, noise_variance, kernel=kern) # Optional: kernel, normalize
m.plot_prior()
