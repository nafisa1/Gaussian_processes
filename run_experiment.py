...
class Experiment(object):
  ...
  def __init__(self, smiles, pic50s, names, descriptors, total_compounds, training_size, test_size, ker, acquisition_function, noise=0.01, number_runs=None, print_interim=False):
    ...
    print number_runs, " compounds out of", self.start_test - training_size, " will be used for optimisation"
    
    self.modopt = model.Model(n_kers=2, Xtrain=[descriptors[:training_size],smiles[:training_size]],Xtest=[descriptors[training_size:self.start_test],smiles[training_size:self.start_test]], Ytrain=pic50s[:training_size], Ytest=pic50s[training_size:self.start_test], kernel=ker)
...
#    modopt.mse_hyperparameters(print_vals=False)  
    ...
    self.modtest = model.Model(n_kers=2, Xtrain=self.modopt.Xtrain, Xtest=[descriptors[-test_size:],smiles[-test_size:]], Ytrain=self.modopt.Ytrain, Ytest=pic50s[-test_size:], kernel=self.modopt.kernel) ## changed kernel from opt to train 


  def bayes_opt(self):
#    self.modopt.kernel.kers[0].sig_var, self.modopt.kernel.kers[0].lengthscale, self.modopt.kernel.kers[1].sig_var, self.modopt.kernel.kers[1].lengthscale = 5,0.8,5,0.8
...
#    self.modtest.Xtest = self.modtest.Xtrain
#    self.modtest.Ytest = self.modtest.Ytrain   

#####################################################################

    # PERFORM REGRESSION ON TEST SET USING INITIAL TRAINING SET
    regtest = self.modtest.regression()
    ...
    for i in xrange(self.number_runs): 
 #     self.modopt.hyperparameters(print_vals=False)
...
#######################?????????????????????
      ...
      newx,newy = self.modopt.optimization()
      
      
      self.modopt.hyperparameters(print_vals=False)
      self.modtest.kernel = self.modopt.kernel
      
      # RESET TRAINING SET OF THE TEST SET MODEL
      self.modtest.Xtrain = self.modopt.Xtrain
      self.modtest.Ytrain = self.modopt.Ytrain
...
#      modopt.mse_hyperparameters(print_vals=False)
      ...
      if i != self.number_runs-1 and self.print_interim == True:
        print "r squared",regtest.r_squared()
        
      # PRINT R SQUARED FOR FINAL RUN
      elif i == self.number_runs-1:
        print "Results using final training set"
        print "r squared",regtest.r_squared()
        print "mse",regtest.mse()
        

#    print "training set mse: ", self.modopt.regression().mse()
    print "test set mse: ", regtest.mse()
    regtest.plot_by_index()
    
#    modopt.regression().plot_by_index()
        
    # ARRAY STARTING AT 0 FOR THE RUN BEFORE OPTIMISATION BEGINS
    run = np.linspace(0,self.number_runs,num=self.number_runs+1)
    
    print "final hps",self.modtest.kernel.kers[0].sig_var, self.modtest.kernel.kers[0].lengthscale, self.modtest.kernel.kers[1].sig_var, self.modtest.kernel.kers[1].lengthscale
...
#    np.savetxt("/home/nafisa/Dropbox/DPhil/Gaussian_processes/results/rsq_" + acquisition_function.abbreviation + "_tr" + str(training_size) + "_te" + str(test_size) + "_runs" + str(number_runs) + ".txt", np.c_[run,r_sq], fmt='%i	%f')
    ...
    return self.modopt,self.modtest
